...
class DatasetConfig:
    ...
    def load(cls, path: Path) -> DatasetConfig:
        ...
        if not license_id or not str(license_data.get("url", "")).startswith("https://"):
            raise ConfigurationError("license provenance ID and URL are required")
        ...
        label_map_version = str(raw["label_map_version"]).strip()
        if label_map_version != "agrimind-cv-binary-label-map-v1":
            raise ConfigurationError("unsupported label-map contract version")
        threshold = raw["near_duplicate_hamming_threshold"]
        if (
            not isinstance(threshold, int)
            or isinstance(threshold, bool)
            or not 0 <= threshold <= 64
        ):
            raise ConfigurationError("near-duplicate threshold must be an integer from 0 to 64")
...
class SplitConfig:
    ...
    contract_version: str = "agrimind-cv-group-split-v1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version != 1 or self.contract_version != "agrimind-cv-group-split-v1":
            raise ConfigurationError("unsupported split contract version")
